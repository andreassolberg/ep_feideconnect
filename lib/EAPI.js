	...
	async.map(fcgroupids, function(fcgroupid, callback) {
		...
		for(i=0; i < results.length; i++) {
			for(j = 0; j < results[i].length; j++) {
				allPads.push(results[i][j]);
				// getPadInfo(results[i][j]);
			}
		}
		...
		async.map(allPads, function(padid, callback) {
			getPadInfo(padid, function(o) {
				callback(null, o);
			})
		}, function(e, res) {
...
var getFCGroupIDs = function(user) {
	...
	for(var key in user.groups) {
		if (user.groups.hasOwnProperty(key)) {
			fcgroupids.push(key);
		}
	}
	// return ['uwap:grp:uninett:org:orgunit:SEK-U23'];
	return fcgroupids;
}
